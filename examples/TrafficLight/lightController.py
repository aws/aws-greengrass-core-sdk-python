...
parser.add_argument(
    "-id", "--clientId", action="store", dest="clientId", default="lightController", help="Targeted client id"
)
...
# This uses the desired property because the light GGAD will get the request for changing and update the
# reported property. The idea is the desired property is a request to update the light while the
# reported property is the actual value of the light
loopCount = 0
states = ["G", "Y", "R"]
pool = cycle(states)
for item in pool:
